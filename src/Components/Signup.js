...
//   const [password, setPassword] = useState();
  ...
  const setStates = async e => {
    ...
    let ml = mobileRef.current.value.length;
    let al = addressRef.current.value.length;

    if (nl === 0 || el === 0 || pl === 0 || ml ===0 || al===0) {
      setVisible(true);
    } else {
    ...
    //    setPassword(passwordRef.current.value);
        ...
        const url = "https://quick-rental-server.herokuapp.com/signup";
